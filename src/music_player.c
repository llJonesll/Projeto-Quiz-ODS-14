 ...
 *
 * @note Mudanças da v2.5:
 * - Slider de volume restaurado para o design original:
 * - Fundo (volume total) em cinza claro.
 * - Preenchimento (volume atual) em amarelo.
 * - **Sem contorno/outline.**
 */
...
void UpdateMusicPlayer(void) {
    // ... (toda a lógica de Update permanece inalterada)
    float deltaTime = GetFrameTime();
    if (!isMusicPaused) UpdateMusicStream(musicPlaylist[currentMusicIndex]);
    if (GetMusicTimeLength(musicPlaylist[currentMusicIndex]) > 0 && 
        GetMusicTimePlayed(musicPlaylist[currentMusicIndex]) >= GetMusicTimeLength(musicPlaylist[currentMusicIndex])) {
        PlayNextSong();
    }
    if (playerState == STATE_ANIMATING_IN) {
        playerAnimationTimer += deltaTime;
        if (playerAnimationTimer >= ANIMATION_SPEED) { playerAnimationTimer = ANIMATION_SPEED; playerState = STATE_SHOWN; }
    } else if (playerState == STATE_ANIMATING_OUT) {
        playerAnimationTimer -= deltaTime;
        if (sliderState != STATE_HIDDEN) sliderState = STATE_ANIMATING_OUT;
        if (playerAnimationTimer <= 0.0f) { playerAnimationTimer = 0.0f; playerState = STATE_HIDDEN; }
    }
    if (sliderState == STATE_ANIMATING_IN) {
        sliderAnimationTimer += deltaTime;
        if (sliderAnimationTimer >= ANIMATION_SPEED) { sliderAnimationTimer = ANIMATION_SPEED; sliderState = STATE_SHOWN; }
    ...
    Rectangle progressBarRec = { infoPanelRec.x + 15, infoPanelRec.y + 80, infoPanelRec.width - 30, 10 };
    ...
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
        if (playerState == STATE_HIDDEN && CheckCollisionPointRec(mousePos, musicIconRec)) {
            ...
            if (sliderState != STATE_HIDDEN) { isClosePending = true; sliderState = STATE_ANIMATING_OUT; } 
            else { playerState = STATE_ANIMATING_OUT; }
            ...
            if (CheckCollisionPointRec(mousePos, muteRec)) {
                if (sliderState == STATE_SHOWN || sliderState == STATE_ANIMATING_IN) {
                    if (musicVolume > 0.01f) { volumeBeforeMute = musicVolume; musicVolume = 0.0f; } 
                    else { musicVolume = volumeBeforeMute; if (musicVolume < 0.01f) musicVolume = 0.5f; }
                    UpdateMusicVolume();
                } else { sliderState = STATE_ANIMATING_IN; }
            ...
            } else if (CheckCollisionPointRec(mousePos, nextRec)) { PlayNextSong();
            } else if (CheckCollisionPointRec(mousePos, prevRec)) { PlayPreviousSong();
    ...
    if (isDraggingVolume) {
        ...
        if (musicVolume > 0.01f) volumeBeforeMute = musicVolume;
        UpdateMusicVolume();
    }
    if (isSeekingSong) {
...
void DrawMusicPlayer(void) {
    ...
    if (playerState != STATE_HIDDEN) {
        ...
        Rectangle progressBarRec = { infoPanelRec.x + 15, infoPanelRec.y + 80, infoPanelRec.width - 30, 6 };
    ...
    if (sliderState != STATE_HIDDEN) {
        ...
        // <<< SLIDER RESTAURADO: Fundo cinza claro, preenchimento amarelo, SEM contorno >>>
        // 1. Desenha o fundo (representando o volume total) em cinza claro.
        DrawRectangleRounded(volumeSliderRec, 0.5f, 4, Fade(LIGHTGRAY, 0.7f * playerProgress));
        
        // 2. Calcula a altura da parte preenchida (volume atual) e desenha em amarelo.
...
// O resto do arquivo (Unload, UpdateVolume, etc.) permanece inalterado.
void UnloadMusicPlayer(void) { for (int i = 0; i < TOTAL_MUSICS; i++) UnloadMusicStream(musicPlaylist[i]); UnloadTexture(texMusicIcon); UnloadTexture(texMute); UnloadTexture(texUnmute); UnloadTexture(texPause); UnloadTexture(texPlay); UnloadTexture(texNext); UnloadTexture(texPrevious); UnloadTexture(texClose); }
void UpdateMusicVolume(void) { SetMusicVolume(musicPlaylist[currentMusicIndex], musicVolume); }
static void ShuffleIntArray(int *array, int size) { for (int i = size - 1; i > 0; i--) { int j = rand() % (i + 1); int temp = array[i]; array[i] = array[j]; array[j] = temp; } }
static void PlayNextSong(void) { StopMusicStream(musicPlaylist[currentMusicIndex]); if (currentPlaylistPosition == -1) { ShuffleIntArray(playlistOrder, TOTAL_MUSICS - 1); currentPlaylistPosition = 0; } else { currentPlaylistPosition++; if (currentPlaylistPosition >= TOTAL_MUSICS - 1) { ShuffleIntArray(playlistOrder, TOTAL_MUSICS - 1); currentPlaylistPosition = 0; } } currentMusicIndex = playlistOrder[currentPlaylistPosition]; PlayMusicStream(musicPlaylist[currentMusicIndex]); UpdateMusicVolume(); if (isMusicPaused) PauseMusicStream(musicPlaylist[currentMusicIndex]); }
static void PlayPreviousSong(void) { if (GetMusicTimePlayed(musicPlaylist[currentMusicIndex]) > 3.0f) { StopMusicStream(musicPlaylist[currentMusicIndex]); PlayMusicStream(musicPlaylist[currentMusicIndex]); UpdateMusicVolume(); if (isMusicPaused) PauseMusicStream(musicPlaylist[currentMusicIndex]); return; } StopMusicStream(musicPlaylist[currentMusicIndex]); if (currentPlaylistPosition <= 0) { currentPlaylistPosition = -1; currentMusicIndex = 0; } else { currentPlaylistPosition--; currentMusicIndex = playlistOrder[currentPlaylistPosition]; } PlayMusicStream(musicPlaylist[currentMusicIndex]); UpdateMusicVolume(); if (isMusicPaused) PauseMusicStream(musicPlaylist[currentMusicIndex]); }
static float Clamp(float value, float min, float max) { if (value < min) return min; if (value > max) return max; return value; }
static const char* FormatTime(float seconds) { if (seconds < 0) seconds = 0; int minutes = (int)seconds / 60; int secs = (int)seconds % 60; static char timeText[6]; sprintf(timeText, "%d:%02d", minutes, secs); return timeText; }