 ...
 *
 * @note Mudanças da v5.3.2 (Correção):
 * - Corrigido bug onde a notificação de pontos não desaparecia da tela.
 * - Restaurado o código que atualiza o `notificationTimer`.
 */
 ...
 void UpdateDrawFrame(void) {
     ...
     // <<< CÓDIGO DO CRONÔMETRO RESTAURADO >>>
     // Handle points notification timer
     if (notificationTimer > 0) {
         notificationTimer -= deltaTime;
         if (notificationTimer < 0) {
             notificationTimer = 0;
             pointsGainedNotification = 0; // Também reseta os pontos para 0
         }
     }
             ...
             if (currentScreen == SCREEN_ENTER_NAME) {
                 ...
                 if (answerTimer <= 0) {
                  ...
                  if (IsKeyPressed(KEY_ENTER)) {
                     PlaySound(victorySfx); currentScreen = SCREEN_LEADERBOARD; ResetWaterFx();
                  }
             }
         } break;
         default: break;
